...
Проект строго опирается на контракты данных Pydantic v2. Для предотвращения сборки pydantic-core из исходников на мобильном CPU рекомендуется использовать предсобранный пакет из репозитория TUR (Termux User Repository):
...
При компиляции из исходников через pip обязательно ограничьте параллелизм сборщика Cargo во избежание сбоя Out-Of-Memory (OOM-killer):
